...
def generate_news() -> None:
    ...
    def make_news_item(meta: dict, body: str) -> dict:
    ...
    years_arch: OrderedDict = OrderedDict()
    ...
    def _years_footer(current_yr: str | None = None) -> str:
        links: list[str] = []
        for y in year_keys_desc:
            links.append(f'<b>{y}</b>' if y == current_yr else f'<a href="/news/{y}/">{y}</a>')
        return ' | '.join(links)
    ...
    if len(all_items) > 10:
        ...
        arch_yr    = ds11[:4]
        arch_mo    = int(ds11[5:7])
        bottom_nav = f'<a href="/news/{arch_yr}/{arch_mo:02d}/">{arch_yr} {MONTHS_RU[arch_mo]} →</a>'
    elif year_keys_desc:
        bottom_nav = f'<a href="/news/{year_keys_desc[0]}/">{year_keys_desc[0]} →</a>'
    else:
        bottom_nav = ''
