...
function init()
{
	...
	//Render the page:
	document.title = msAlbumTitle.replace(/\n/g, '');
	...
	while (n < mnTot)
	{
		...
		for (i = 0; i < nCols; i++)
		{
			...
			if (n < mnTot)
			{
				...
				sHtml += '<img src="T/' + msImage[n] + '.jpg" height=' + mnThumH[n] + ' width=' + mnThumW[n] + ' border=0 hspace=0 vspace=0 title="' + msDescr[n] + '" style="border:1px solid ' + msBorderColor + '" onmouseover="this.style.borderColor=\'' + msTextColor + '\'" onmouseout="this.style.borderColor=\'' + msBorderColor + '\'">';
				sHtml += '</a>';
				if (msTitle[n] != '')
